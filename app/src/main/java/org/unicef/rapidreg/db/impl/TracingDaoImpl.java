...
public class TracingDaoImpl implements TracingDao {
    ...
    public Tracing getTracingByUniqueId(String uniqueId) {
        return SQLite.select().from(Tracing.class).where(Tracing_Table.unique_id.eq(uniqueId))
                .querySingle();
    }
    ...
    @Override
    public List<Tracing> getAllTracingsOrderByAge(boolean isASC) {
        return isASC ? getTracingsByAgeASC() : getTracingsByAgeDES();

    }

    @Override
    public List<Tracing> getTracingListByConditionGroup(ConditionGroup conditionGroup) {
    ...
    private List<Tracing> getTracingsByAgeASC() {
        return SQLite.select().from(Tracing.class).orderBy(Tracing_Table.age, true).queryList();
    }

    private List<Tracing> getTracingsByAgeDES() {
        return SQLite.select().from(Tracing.class).orderBy(Tracing_Table.age, false).queryList();
    }
